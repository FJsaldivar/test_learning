...
public class BaseApiManager {
        ...
        return Promise { seal in
            AF.request(request)
                ...
                .responseJSON { response in
                    ...
                    case .failure(let error):
                        // If there is no data in the response retrieve the generic error
                        guard let data = response.data else {
                            seal.reject(error)
                            return
                        }
                        
                        // Map the error as `UalaError`
                        if let error: UalaError = router.errorMapper.map(data) {
                            seal.reject(error)
                        // If  previous map fails means the error is not being handled
                        // in the 'UalaError' enum, hence we map the 'CustomError'
                        } else if let error: BEError = router.errorMapper.map(data) {
                            seal.reject(error)
                        }
                    }
            }
    ...
    public func publicRequest(_ router: Routeable, body: Data? = nil) -> Promise<Void> {
