...
public class UalaValidator: PasswordValidator {
    ...
    public func validate(document: String) -> ValidationResult {
        ...
        rules.add(rule: NoWhiteSpacesValidationRule(error: UalaError.passwordWithSpaces))
        rules.add(rule: ValidationRulePattern(pattern: UalaValidationPattern.numbers, error: UalaError.dniInvalidFormat))
    ...
    public func validate(cbuCount: String) -> ValidationResult {
        var rules = ValidationRuleSet<String>()
        rules.add(rule: ValidationRuleLength(min: 22, max: 22, error: UalaError.cbuInvalidCount))
    ...
    public func validate(paymentBarCode: String) -> ValidationResult {
        var rules = ValidationRuleSet<String>()
        rules.add(rule: ValidationRulePattern(pattern: UalaValidationPattern.lettersAndNumbers, error: UalaError.barCodeFormatError))
        rules.add(rule: ValidationRuleLength(min: 8, error: UalaError.barCodeFormatLengthError))

        return paymentBarCode.validate(rules: rules)
    }
}
