...
public class PhoneNumbersPrefix {
    ...
    public func getPrefix(phoneNumber: String) -> String {
        let number = removeUnnecessaryChars(from: phoneNumber)
        for count in [4, 3, 2] {
            let index = number.index(phoneNumber.startIndex, offsetBy: count)
            let newPrefix = number.substring(to: index)
            let result = prefixes.contains(newPrefix)
    ...
    public func getNumber(phoneNumber: String) -> String {
        var number = removeUnnecessaryChars(from: phoneNumber)

        let prefix = getPrefix(phoneNumber: phoneNumber)
        
        number = number.chopPrefix(prefix.count)
        
        if String(number.prefix(2)) == "15" {
    ...
    public func getNumberWithoutAreaCode(phoneNumber: String) -> String {
        let number = removeUnnecessaryChars(from: phoneNumber)

        return number.substring(from: number.index(number.endIndex, offsetBy: -8))
    }
    
    public func removeCero(from prefix: String) -> String {
        let first = String(prefix.prefix(1))
        if first == "0" {
    ...
    private func removeUnnecessaryChars(from number: String) -> String {
        let numberWithOutCountryCode = number.components(separatedBy: .whitespaces).joined().replacingOccurrences(ofStrings: [" ", "(", ")", "-", "+549", "+54"], with: "")
        return removeCero(from: numberWithOutCountryCode)
    }
}
