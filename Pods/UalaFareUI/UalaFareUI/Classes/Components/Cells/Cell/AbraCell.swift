...
/// The main cell used for displaying information, it uses the AbraCellType to decide what info is going to display
final public class AbraCell: UITableViewCell {
    private lazy var mainStackView: UIStackView = {
        return configureStackView()
    }()
    
    /// Returns if the checkbox is selected in case that exists, otherwise returns false
    public var isCheckboxSelected: Bool {
        return checkbox?.isCheckboxSelected ?? false
    }
    
    /// Returns if the switch is on in case that exists, otherwise returns false
    public var isToggleOn: Bool {
        return toggle?.isOn ?? false
    }
    
    /// Returns if the radio button is selected in case that exists, otherwise returns false
    public var isRadioSelected: Bool {
        return radioButton?.isRadioSelected ?? false
    }
    
    private var separator: UIView?
    private weak var delegate: AbraCellDelegate?
    private var indexPath: IndexPath?
    private weak var checkbox: AbraCheckbox?
    private weak var toggle: AbraSwitch?
    private weak var radioButton: AbraRadioButton?
    
    /// This function set the delegate to the cell so it can receive events from the cell
    public func setDelegate(delegate: AbraCellDelegate, indexPath: IndexPath) {
        self.delegate = delegate
        self.indexPath = indexPath
    }
    
    public func configure(with model: AbraCellThemedModel) {
        configureCell()
    ...
    private func configureSeparatorIfNeeded(model: AbraCellThemedModel) {
    ...
    private func configureRadioButton(insertInto stackView: UIStackView, theme: AbraRadioButtonTheme, size: ComponentSize) {
        let width: CGFloat = size == .large ? 24 : 16
        let radioButton = AbraRadioButton(frame: CGRect(x: 0,
                                                          y: 0,
                                                          width: width,
                                                          height: width),
                                          theme: theme)
    ...
    private func configureSwitch(insertInto stackView: UIStackView, model: AbraSwitchTheme) {
        let toggleSwitch = AbraSwitch(theme: model, frame: CGRect(x: 0, y: 0, width: 48, height: 24))
    ...
    private func configureCheckbox(model: AbraCheckboxTheme,
                                   size: ComponentSize,
                                   insertInto stackView: UIStackView) {
        let checkboxWidth: CGFloat = size == .small ? 16 : 24
        let checkBox = AbraCheckbox(model: model, frame: CGRect(x: 0,
                                                              y: 0,
                                                              width: checkboxWidth,
    ...
    private func configureButton(text: String?, icon: UIImage?, addInto stackview: UIStackView, theme: AbraCellTheme) {
    ...
    private func configureRightStackIfNeeded(with model: AbraCellThemedModel) {
    ...
    private func configureLeftStackViewIfNeeded(with model: AbraCellThemedModel) {
    ...
    private func configureBulletNumber(number: Int, theme: AbraCellTheme, addInto stackView: UIStackView) {
    ...
    private func configureIcon(icon: AbraCellIconType,
                               theme: AbraCellTheme,
                               isRounded: Bool,
    ...
    private func configureCentralStackView(model: AbraCellThemedModel) {
    ...
    @objc private func checkboxStatusUpdated(_ checkbox: AbraCheckbox) {
        guard let delegate = delegate, let indexPath = indexPath else {
            return
        }
