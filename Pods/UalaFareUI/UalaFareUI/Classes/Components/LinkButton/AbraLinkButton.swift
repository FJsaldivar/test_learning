...
/// A button with an image indicating that it is a link to an external website
final public class AbraLinkButton: UIButton {
    ...
    private let theme: AbraButtonTheme
    
    /**
     - Parameters:
        - customFrame: The frame of the button, the default is zero
     - Returns: A button formatted for external links
     */
    public convenience init(customFrame: CGRect = .zero) {
        let theme = AbraThemeManager.theme.buttonTheme(for: .link)
        self.init(frame: customFrame, theme: theme)
    }
    
    /**
     - Parameters:
        - frame: The frame of the button, the default is zero
        - theme: The theme of the button
     - Returns: A button formatted for external links
     */
    public init(frame: CGRect = .zero,
                theme: AbraButtonTheme) {
        self.theme = theme
    ...
    public override init(frame: CGRect) {
        theme = AbraThemeManager.theme.buttonTheme(for: .link)
        super.init(frame: frame)
        setLinkIcon(linkIconImage)
        configureTitleLabel()
    }
